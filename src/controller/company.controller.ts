...
export class CompanyController {
  ...
  async createCompany(req: CustomRequest, res: Response, next: NextFunction) {
    try {
  ...
  async getCompanyById(req: CustomRequest, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
  ...
  async updateCompany(req: CustomRequest, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const company: ICompanyRequest = req.body;
      const { value, error } = schemaCreateCompany.validate(company);
  ...
  async deleteCompany(req: CustomRequest, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      await this.companyService.deleteCompany(id);
      res.status(httpStatus.NO_CONTENT).send();
    } catch (error) {
      return next(errorHandler("deleteCompany", error));
    }
  }
}
