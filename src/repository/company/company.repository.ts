import { ICompanyRequest, ICompanyRequestUpdate, ICompanyResponse } from "../../model/company/company.interface.ts";
...
export class CompanyRepository implements ICompanyRepository {
    ...
    async findByEmail(email: string): Promise<ICompanyResponse> {
        const companyFound = await company.findOne({ email }) as ICompanyResponse
        if (!companyFound) {
